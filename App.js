...
const [dailyBonusClaimed, setDailyBonusClaimed] = useState(false);
  ...
  // Keeps the latest rolling state available to websocket callbacks without stale React state.
    ...
    (async () => {
      try {
        ...
        if (saved) {
          ...
          // Keep the ref in sync immediately so pending atomic transactions can
          // be reconciled before any cloud upsert writes an old local balance.
          currentUserRef.current = parsed;
          setCurrentUser(parsed);
          if (parsed.avatar) setUserAvatar(parsed.avatar);

          const reconciled = await reconcilePendingCoinTransactions();
          if (reconciled) {
            await syncUserToCloud(currentUserRef.current || parsed);
          } else {
            console.warn('Pending coin transaction could not be reconciled yet.');
          }
        }
      } catch (err) {}
    })();
...
const joinAgoraVoiceChannel = async () => {
  ...
  // ========== HELPER FUNCTIONS ==========
  // Coin deductions must be committed on Supabase first. The app never
  // changes the local balance until the backend confirms the atomic result.
  const COIN_TX_PREFIX = '@ludo_coin_tx_';

  const makeCoinOperationId = () =>
    `${Date.now()}_${Math.random().toString(36).slice(2)}_${Math.random().toString(36).slice(2)}`;

  const callAtomicCoinDeduction = async (transaction) => {
    const response = await fetch(`${SUPABASE_REST_URL}/rpc/deduct_coins_atomic`, {
      method: 'POST',
      headers: {
        ...supabaseHeaders,
        Prefer: 'return=representation'
      ...
      body: JSON.stringify({
        p_player_id: String(transaction.playerId),
        p_amount: Number(transaction.amount),
        p_operation_id: String(transaction.operationId),
        p_reason: String(transaction.reason || 'COIN_DEDUCTION')
      })
    });
    ...
    if (!response.ok) {
      const message = result?.message || result?.hint || raw || `HTTP ${response.status}`;
      throw new Error(message);
    }

    // PostgREST can return an object or an array depending on the function shape.
    return Array.isArray(result) ? result[0] : result;
  };

  const reconcilePendingCoinTransactions = async () => {
    const user = currentUserRef.current;
    if (!user?.playerId) return true;

    const pendingRaw = await AsyncStorage.getItem(`${COIN_TX_PREFIX}${user.playerId}`);
    ...
    for (const transaction of pending) {
      try {
        const result = await callAtomicCoinDeduction(transaction);
        if (!result?.success) {
  ...
  const deductUserCoins = async (amount, reason = 'COIN_DEDUCTION') => {
    const user = currentUserRef.current;
    if (!user?.playerId) return false;

    // First resolve any transaction that may have been interrupted by an app
    // close or a network drop. Reusing the same operation ID is idempotent.
    const reconciled = await reconcilePendingCoinTransactions();
    if (!reconciled) {
      Alert.alert(
        'Transaction Pending',
        'A previous coin transaction is still syncing. Please check your internet connection and try again.'
      );
      return false;
    }

    const currentCoins = Number(currentUserRef.current?.coins || 0);
    ...
    const transaction = {
      playerId: String(currentUserRef.current.playerId),
      amount: Number(amount),
      operationId: makeCoinOperationId(),
      reason,
      createdAt: new Date().toISOString()
    };

    const pendingKey = `${COIN_TX_PREFIX}${transaction.playerId}`;
    let pending = [];
    try {
    ...
    // Persist before the network request. If the app closes after Supabase
    // commits, the same operation ID can safely be retried on next launch.
    ...
    try {
      ...
      if (!result?.success) {
        if (result?.code === 'INSUFFICIENT_COINS') {
          Alert.alert('Low Coins', 'You do not have enough coins for this action.');
          pending = pending.filter(t => t.operationId !== transaction.operationId);
          await AsyncStorage.setItem(pendingKey, JSON.stringify(pending));
        } else {
          Alert.alert('Transaction Failed', result?.message || 'Coin deduction could not be completed.');
        }
        return false;
      }
      ...
      console.warn('Atomic coin deduction error:', e);
      Alert.alert(
        'Transaction Pending',
        'Network error. Coins were not changed locally. The transaction will safely retry when the app reconnects.'
      );
      return false;
    }
  };
...
const handleTimeoutMiss = () => {
  ...
  if (
    ...
    // Final result pehle decide karo. Isse dice ke animation ke last moment par
    // ek naya random result generate nahi hoga.
    const finalVal = Math.floor(Math.random() * 6) + 1;
    const shuffleSteps = [50, 70, 90, 110];

    for (let i = 0; i < shuffleSteps.length; i++) {
      // Last visible animation frame ko final result hi rakho.
      const rand = i === shuffleSteps.length - 1
        ? finalVal
      ...
      playerDicesRef.current = nextDices;
      setPlayerDices(nextDices);
      await sleep(shuffleSteps[i]);
    }

    // Ref ke latest snapshot se final state banao, stale playerDices state se nahi.
    const newDices = {
      ...playerDicesRef.current,
      [currentTurn]: finalVal
    };
  ...
  useEffect(() => {
    ...
    const connectSocket = () => {
      ...
      socket.onopen = () => {
        ...
        if (!wasReconnecting) setIsReconnecting(false);
        socket.send(JSON.stringify({ topic: `realtime:room_${roomCode}`, event: 'phx_join', payload: {}, ref: 'room_join_ref' }));
        // Ask the room for the latest board state after a reconnect.
        socket.send(JSON.stringify({
          topic: `realtime:room_${roomCode}`,
          event: 'broadcast',
          payload: { type: 'REQUEST_SYNC', data: { requesterColor: myColorRef.current } },
          ref: `sync_request_${Date.now()}`
        }));
    ...
    socket.onmessage = async (e) => {
      try {
        ...
        if (type === 'CHECK_ROOM_EXISTS') {
          ...
          // Any currently connected player can restore the returning player's latest state.
          ...
          if (!isHostRef.current) {
            await deductUserCoins(data.entryFee || 50, 'MATCH_ENTRY_FEE');
          }
          ...
          if (data.updatedDices) {
            // Never let a delayed multiplayer SYNC overwrite the local dice
            // while its animation/final result is still being completed.
      ...
      socket.onclose = () => {
        if (intentionallyClosed || !roomCodeRef.current) return;
        setIsReconnecting(true);
        const nextAttempt = reconnectAttemptRef.current + 1;
        reconnectAttemptRef.current = nextAttempt;
        setReconnectAttempt(nextAttempt);
        const delay = Math.min(1000 * Math.pow(2, Math.min(nextAttempt - 1, 3)), 10000);
        reconnectTimerRef.current = setTimeout(connectSocket, delay);
      };

      socket.onerror = () => {
        // onclose schedules the retry so only one reconnect loop exists.
      };
    };
...
useEffect(() => {
  ...
  const startMatchFromLobby = async () => {
    ...
    const canPlay = await deductUserCoins(selectedEntryFee, 'MATCH_ENTRY_FEE');
    if (!canPlay) return;
...
if (!hasPermission) {
      ...
      Alert.alert(
        ...
        [
          ...
          { text: 'Pay 500 Coins', onPress: async () => {
            const success = await deductUserCoins(500, 'VOICE_CHAT_UNLOCK');
            if (success) {
  ...
  const getPlayerLabelPositionStyle = (color) => {
    // Keep every player name on a horizontal edge of its own base.
    // This must depend on the base/player color, not on the local host color.
    switch (color) {
      case 'RED':
      case 'GREEN':
        return styles.playerLabelTop;
      case 'BLUE':
      case 'YELLOW':
      default:
        return styles.playerLabelBottom;
    }
  };
...
{isPlayable && (
  <Text
    style={[
      styles.playerLabel,
      getPlayerLabelPositionStyle(color),
...
>
  ...
  return (
    <SafeAreaView style={styles.mainContainer}>
      ...
      {(gameMode === 'ONLINE' || gameMode === 'HYBRID') && isReconnecting && (
        <View pointerEvents="none" style={styles.reconnectingBanner}>
          <Text style={styles.reconnectingBannerText}>📶 Reconnecting... Syncing game</Text>
          {reconnectAttempt > 0 && <Text style={styles.reconnectingAttemptText}>Attempt {reconnectAttempt}</Text>}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  reconnectingBanner: { position:'absolute', top:70, left:18, right:18, zIndex:100, backgroundColor:'rgba(120,53,15,0.96)', borderWidth:1.5, borderColor:'#facc15', borderRadius:12, paddingVertical:8, paddingHorizontal:12, alignItems:'center', elevation:15 },
  reconnectingBannerText: { color:'#ffffff', fontSize:13, fontWeight:'900', letterSpacing:0.3 },
  reconnectingAttemptText: { color:'#fde68a', fontSize:10, fontWeight:'700', marginTop:2 },
