...
// 3D Teardrop Goti with Base Pedestal Ring
const PinToken = ({ colorHex, rotateBack, stackCount, inBase = false }) => (
...
export default function App() {
  ...
  }, [chatModal, avatarModal, friendsModal, profileStatsModal, settingsModal, botSelectModal, passPlayModal, hybridTeamModal, onlineScreen, onlineLobbyModal, gameMode]);
    ...
    try {
      ...
      if (pairs && Array.isArray(pairs)) {
        ...
        if (usersData && Array.isArray(usersData)) {
          const list = usersData.map(u => ({ id: u.player_id, name: u.name, online: true, avatar: u.avatar || '👦' }));
          setFriendsList(list);
        }
      }
    } catch (e) {}
    ...
    ALL_COLORS.forEach((color) => {
      pawns[color].forEach((stepCount, idx) => {
        ...
        rendered.push(
          ...
          >
            <PinToken colorHex={colorHex} rotateBack={shouldFlipBoard} stackCount={stackCount} inBase={stepCount === -1} />
          </TouchableOpacity>
        );
      });
    });
            ...
            <View style={styles.inviteModalOverlay}>
              <View style={styles.glassCard}>
                ...
                <View style={styles.tabToggleRow}>
                  ...
                  <TouchableOpacity style={[styles.tabToggleBtn, friendsTab === 'INVITES' && styles.tabToggleActive]} onPress={() => setFriendsTab('INVITES')}>
                    <Text style={[styles.tabToggleText, friendsTab === 'INVITES' && styles.tabToggleTextActive]}>🎮 Invites ({incomingInvitesList.length})</Text>
                  </TouchableOpacity>
                ...
                {friendsTab === 'INVITES' && (
                  <ScrollView style={{ maxHeight: 200, marginTop: 10, width: '100%' }}>
                    {incomingInvitesList.length === 0 ? (
                      <Text style={{ color: '#94a3b8', textAlign: 'center', marginTop: 14 }}>No match invites received.</Text>
                    ) : (
                      incomingInvitesList.map((inv) => (
                        <View key={inv.id} style={styles.friendCardRow}>
                          <View>
                            <Text style={styles.friendNameText}>🎮 {inv.from_name}</Text>
                            <Text style={styles.friendStatusText}>Room: #{inv.room_code} (🪙 {inv.entry_fee})</Text>
                          </View>
                          <TouchableOpacity 
                            style={[styles.invitePillBtn, { backgroundColor: '#eab308' }]} 
                          ...
                          >
                            <Text style={[styles.invitePillBtnText, { color: '#000000' }]}>✓ Join</Text>
                          </TouchableOpacity>
                        </View>
                      ))
                    )}
                    ...
                    {friendsList.length === 0 ? (
                      ...
                      friendsList.map((friend) => (
                        <View key={friend.id} style={styles.friendCardRow}>
                          ...
                          <TouchableOpacity style={styles.invitePillBtn} onPress={() => sendFriendInvite(friend)}>
...
const styles = StyleSheet.create({
  ...
  friendsTouchBtn: { backgroundColor: '#0284c7', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: '#38bdf8', position: 'relative' },
  ...
  // 3D Teardrop Token Pin Styling with Pedestal Ring
  ...
  // Clean Crisp Cell Borders Matching Reference Layout
