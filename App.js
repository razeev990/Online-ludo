...
// ✅ FIXED: BASE_SPOTS - tokens now perfectly centered
const BASE_SPOTS = {
...
useEffect(() => {
  const onBackPress = () => {
    if (gameMode) {
      // If game is running, show exit confirmation
      handleExitGame();
      return true; // Prevent default behavior (app exit)
    }
    return false; // Let default behavior happen (allow app to go background)
  };
...
const claimDailyBonus = async () => {
  ...
  try {
    ...
    // ✅ Daily bonus reward changed to 200
    const reward = 200;
