...
export default function App() {
  ...
  const getStrategicMoveIndex = (color, diceVal, validMoves) => {
    ...
    // Priority 3: Reach Safe Spot (Star)
    for (let idx of validMoves) {
      let step = playerPawns[idx];
      let targetStep = step === -1 ? 0 : step + diceVal;
      if (targetStep < 51) {
    ...
    // Priority 6: Balanced progression
  ...
  const startMatchFromLobby = async () => {
    ...
    const currentJoinedCount = Object.keys(roomPlayers).length;
    if (currentJoinedCount < activeColors.length) {
      Alert.alert('Waiting for Players', `Please wait for all players to join (${currentJoinedCount}/${activeColors.length}).`);
      return;
    }
  ...
  const finalizeMove = (color, index, finalStep, diceVal, finalState, currentDices) => {
    ...
    let extraTurn = diceVal === 6 || finalStep === 56; // 6 aane par ya goti ghar mein jaane par extra turn
  ...
  if (onlineLobbyModal) {
    ...
    const currentJoinedCount = Object.keys(roomPlayers).length;
    const isRoomFull = currentJoinedCount === activeColors.length;
          ...
          {isHost ? (
            ...
            >
              <Text style={[styles.startMatchGoldText, !isRoomFull && { color: '#94a3b8' }]}>
                {isRoomFull ? 'START MATCH NOW ➔' : `WAITING FOR PLAYERS (${currentJoinedCount}/${activeColors.length})...`}
              </Text>
            </TouchableOpacity>
          ) : (
...
const styles = StyleSheet.create({
  ...
  boardContainer: { width: BOARD_SIZE, height: BOARD_SIZE, backgroundColor: '#ffffff', borderWidth: 3, borderColor: '#0f172a', borderRadius: 16, overflow: 'hidden', elevation: 12, shadowColor: '#000', shadowOpacity: 0.5, shadowRadius: 10 },
  board: { width: '100%', height: '100%', position: 'relative', backgroundColor: '#ffffff' },
  cell: { position: 'absolute', width: CELL_SIZE, height: CELL_SIZE, borderWidth: 0.5, borderColor: '#cbd5e1', justifyContent: 'center', alignItems: 'center' },
  ...
  base: { position: 'absolute', width: CELL_SIZE * 6, height: CELL_SIZE * 6, justifyContent: 'center', alignItems: 'center', padding: 8, borderRadius: 16 },
  ...
  baseInnerWhite: { width: '84%', height: '84%', backgroundColor: '#ffffff', borderRadius: 14, justifyContent: 'space-around', padding: 8 },
  pocketRow: { flexDirection: 'row', justifyContent: 'space-around' },
  basePocket: { width: 28, height: 28, borderRadius: 14 },
