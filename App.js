...
export default function App() {
  ...
  const deductUserCoins = async (amount) => {
    ...
    if (currentUser.coins < amount) {
      Alert.alert('Low Coins', `You need at least 🪙 ${amount} coins. Claim your daily free reward from the main lobby!`);
      return false;
    }
  ...
  const claimFreeBonus = async () => {
    ...
    setCurrentUser(updatedUser);
    await AsyncStorage.setItem('@ludo_supreme_user', JSON.stringify(updatedUser));
    Alert.alert('🎁 Bonus Claimed!', '🪙 200 Free Coins added to your account!');
  };
  ...
  // Send Text or Emoji Chat Message
  ...
  const handlePlayerElimination = (eliminatedColor) => {
    ...
    if (remaining.length <= 1) {
      ...
      Alert.alert('Player Disqualified', `${eliminatedColor} missed 4 chances and was removed.`);
      sendMultiplayerSync(pawnsRef.current, nextIdx, playerDices, false);
    }
  };

  const getBestMoveIndex = (color, diceVal, validMoves) => {
    ...
    return validMoves.reduce((best, curr) => (playerPawns[curr] > playerPawns[best] ? curr : best), validMoves[0]);
  };

  const rollDiceAuto = async () => {
    ...
    if (validMoves.length === 0) {
      ...
      const bestMove = getBestMoveIndex(currentTurn, finalVal, validMoves);
      setTimeout(() => executeStepMovement(currentTurn, bestMove, finalVal, newDices), 300);
    }
  };
      ...
      try {
        ...
        if (msg.event === 'broadcast' && msg.payload) {
          ...
          if (type === 'GAME_INVITE') {
            ...
            const newReq = { id: data.fromId, name: data.fromName, email: data.fromEmail || '' };
            setPendingRequests((prev) => {
              const updated = [...prev.filter(r => r.id !== data.fromId), newReq];
              saveUserData(currentUser.playerId, friendsList, updated);
              return updated;
            });
            ...
            Alert.alert('Request Accepted!', `${data.fromName} accepted your friend request!`);
          }
        }
      } catch (err) {}
    };
  ...
  const handleSearchUser = async () => {
    ...
    try {
      ...
      for (let email in dbUsers) {
        const u = dbUsers[email];
        if (email === query || u.playerId === query || u.name.toLowerCase() === query) {
          foundUser = u;
          break;
        }
      }
      ...
      if (foundUser) {
        if (foundUser.playerId === currentUser.playerId) {
          Alert.alert('Note', 'This is your own profile ID.');
          setSearchedUserResult(null);
        } else {
          setSearchedUserResult(foundUser);
        }
      } else {
  ...
  const sendRealtimeFriendRequest = (targetUser) => {
    if (userWs.current && userWs.current.readyState === WebSocket.OPEN) {
      ...
      Alert.alert('Error', 'Could not connect to network. Try again.');
    }
  };

  const acceptFriendRequest = (reqUser) => {
  ...
  const joinOnlineRoom = () => {
    ...
    joinTimeoutRef.current = setTimeout(() => {
      tempWs.close();
      setIsVerifyingRoom(false);
      Alert.alert('Room Not Found', 'No active host found with this code. Please check the room code.');
    }, 4500);
  };
  ...
  useEffect(() => {
    const isBotTurn = (gameMode === 'BOT' && currentTurn !== 'BLUE') || 
                     ...
                     (playerMissCount[currentTurn] >= 3);

    if (isBotTurn && !hasRolled && !isRolling && !isMoving && !showPodiumBoard) {
      const botTimer = setTimeout(() => rollDiceAuto(), 800);
      return () => clearTimeout(botTimer);
    }
  }, [turnIndex, hasRolled, isMoving, gameMode, showPodiumBoard, currentTurn, playerSlots, playerMissCount]);
  ...
  const rollDice = async (isBot = false) => {
    ...
    Animated.parallel([
      ...
      Animated.sequence([
        Animated.timing(diceBounceAnim, { toValue: 1.35, duration: 200, useNativeDriver: false }),
        Animated.timing(diceBounceAnim, { toValue: 0.85, duration: 200, useNativeDriver: false }),
        Animated.timing(diceBounceAnim, { toValue: 1, duration: 300, useNativeDriver: false }),
      ])
    ]).start();
    ...
    } else if (validMoves.length === 1 || (gameMode === 'BOT' && currentTurn !== 'BLUE') || (gameMode === 'HYBRID' && playerSlots[currentTurn] === 'BOT') || (playerMissCount[currentTurn] >= 3)) {
      const bestMove = getBestMoveIndex(currentTurn, finalVal, validMoves);
  ...
  const renderCell = (row, col) => {
    ...
    return (
      <View key={`${row}-${col}`} style={[styles.cell, { left: col * CELL_SIZE, top: row * CELL_SIZE, backgroundColor: bgColor }]}>
        {isStar && <Text style={[styles.starText, shouldFlipBoard && { transform: [{ rotate: '180deg' }] }]}>☆</Text>}
        {isArrow && <Text style={[styles.arrowText, shouldFlipBoard && { transform: [{ rotate: '180deg' }] }]}>{arrowIcon}</Text>}
      </View>
    );
  };
      ...
      pawns[color].forEach((stepCount, idx) => {
        ...
        if (stepCount >= 0 && stepCount < 56) {
          ...
          if (stackCount > 1) {
            ...
            if (indexInGroup === 0) { offsetX = -3; offsetY = -2; }
            else if (indexInGroup === 1) { offsetX = 4; offsetY = 2; }
            else if (indexInGroup === 2) { offsetX = 0; offsetY = 4; }
            else if (indexInGroup === 3) { offsetX = -3; offsetY = 4; }
          }
        }
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              { 
                ...
                top: coords[0] * CELL_SIZE - 6 + offsetY,
          ...
          {isLeftDice ? (
            <>
              ...
              <View style={styles.cardAvatarRight}>
                ...
                {misses > 0 && <Text style={styles.missCounterBadge}>⚠️ {misses}/4</Text>}
        ...
        <ScrollView style={{ width: '100%' }} contentContainerStyle={{ alignItems: 'center', paddingBottom: 20 }}>
          ...
          <View style={[styles.glassCard, { marginTop: 14 }]}>
            ...
            <View style={styles.prizePoolPreviewBox}>
              <Text style={styles.prizePoolPreviewLabel}>🛡️ RISK FREE MATCH (NO COIN LOSS):</Text>
              <Text style={styles.prizePoolPreviewAmount}>🪙 Win 300 Coins on 1st Place!</Text>
            </View>
          ...
          <View style={styles.teamMatchLobbyWrap}>
            <View style={styles.teamLobbyBoxA}>
              ...
              <View style={styles.teamSlotsRow}>
                ...
                <TouchableOpacity
                  activeOpacity={0.8}
                  style={[styles.slotInviteBox, (roomPlayers['GREEN'] || playerSlots['GREEN'] === 'BOT') && styles.slotBoxFilledGreen]}
                  onPress={() => !roomPlayers['GREEN'] && playerSlots['GREEN'] !== 'BOT' && setFriendsModal(true)}
                >
                  {playerSlots['GREEN'] === 'BOT' ? (
                    <>
            ...
            <View style={styles.teamLobbyBoxB}>
              ...
              <View style={styles.teamSlotsRow}>
                <TouchableOpacity
                  activeOpacity={0.8}
                  style={[styles.slotInviteBox, (roomPlayers['RED'] || playerSlots['RED'] === 'BOT') && styles.slotBoxFilledRed]}
                  onPress={() => !roomPlayers['RED'] && playerSlots['RED'] !== 'BOT' && setFriendsModal(true)}
                >
                  {playerSlots['RED'] === 'BOT' ? (
                    <>
                      <Text style={{ fontSize: 32 }}>🤖</Text>
                ...
                <TouchableOpacity
                  activeOpacity={0.8}
                  style={[styles.slotInviteBox, (roomPlayers['YELLOW'] || playerSlots['YELLOW'] === 'BOT') && styles.slotBoxFilledYellow]}
                  onPress={() => !roomPlayers['YELLOW'] && playerSlots['YELLOW'] !== 'BOT' && setFriendsModal(true)}
                >
                  {playerSlots['YELLOW'] === 'BOT' ? (
                    <>
                      <Text style={{ fontSize: 32 }}>🤖</Text>
                      <Text style={styles.slotPlayerNameText}>AI Bot</Text>
  ...
  return (
    <SafeAreaView style={styles.mainContainer}>
      ...
      {/* REALTIME IN-GAME CHAT & EMOJI MODAL */}
      ...
      <View style={styles.topCardsRow}>
        {shouldFlipBoard ? (
          <>
            ...
            {renderPlayerCard('BLUE', '#0284c7', '#0369a1', true, true)}
          </>
        ) : (
          <>
      ...
      <View style={styles.bottomCardsRow}>
        {shouldFlipBoard ? (
          <>
            {renderPlayerCard('GREEN', '#16a34a', '#14532d', false, false)}
            {renderPlayerCard('RED', '#e11d48', '#991b1b', true, false)}
          </>
        ) : (
          <>
...
const styles = StyleSheet.create({
  ...
  topLeftProfileWrap: { marginLeft: 10, paddingVertical: 4, paddingHorizontal: 6, justifyContent: 'center' },
