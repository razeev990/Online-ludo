...
export default function App() {
  ...
  // Board Flip fix: Online/Hybrid matches mein board kabhi bhi flip nahi hoga taki goti aur cells match karein
  ...
  // Fix: 30s timeout hone par pehle dice auto-roll hoga, fir AI best move chalega aur 3 miss par elimination
  ...
  // Fix: Human-like smart AI strategy (Cut enemy, Safe Star, Home path, Balance pawns)
  ...
  // Fix: Prevent duplicate invite spam and show "Invited" instantly
        ...
        if (message.event === 'broadcast') {
          ...
          if (type === 'CHECK_ROOM_EXISTS') {
            ...
            if (activeColors.length <= 2) {
              ...
              Alert.alert('Player Disconnected', `${leftName} has left the match. Game continues without them.`);
  ...
  const renderAllTokens = () => {
    ...
    ALL_COLORS.forEach((color) => {
      pawns[color].forEach((stepCount, idx) => {
        ...
        if (shouldFlipBoard) {
          finalLeft = (14 - coords[1]) * CELL_SIZE - offsetX;
          finalTop = (14 - coords[0]) * CELL_SIZE - 7 - offsetY;
        }
        ...
        >
          {isLeftDice ? (
            <>
              ...
              {/* Timer positioned in center gap */}
              {isCurrent && (
...
const styles = StyleSheet.create({
  ...
  rankPlayerName: { fontSize: 13, fontWeight: 'bold', flex: 1, marginLeft: 6, color: '#ffffff' }, // Fixed text contrast for Leaderboard names
