...
export default function App() {
  ...
  // Real-time Online/Offline Status Tracking for Friends
  useEffect(() => {
    ...
    updatePresence(true);
    const heartbeatInterval = setInterval(() => updatePresence(true), 25000);
  ...
  // Real-time Online/Offline Status Integration for Friends List
      ...
      if (pairs && Array.isArray(pairs)) {
        ...
        if (usersData && Array.isArray(usersData)) {
          ...
          const list = usersData.map(u => {
            ...
            const isOnline = (now - lastSeenTime) < 45000; // 45 seconds threshold for active status
            return { 
              id: u.player_id, 
              name: u.name, 
              online: isOnline, 
  ...
  // Friends Modal with Live Online/Offline Status Indicator
    ...
    <Modal transparent animationType="slide" visible={friendsModal}>
      <View style={styles.inviteModalOverlay}>
        <View style={[styles.glassCard, { maxHeight: '92%', padding: 14 }]}>
          ...
          {/* Friends List Tab with Real-time Online/Offline Status */}
          {friendsTab === 'LIST' && (
                  ...
                  <View key={friend.id} style={styles.friendItemCardWrap}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      ...
                      <View style={{ marginLeft: 10 }}>
                        ...
                        <Text style={styles.friendCardSubText}>ID: #{friend.id} • <Text style={{ color: friend.online ? '#4ade80' : '#94a3b8', fontWeight: 'bold' }}>{friend.online ? 'Online' : 'Offline'}</Text></Text>
                      </View>
                    </View>
