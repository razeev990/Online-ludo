...
const getPerspectiveOffset = (color) => {
  switch (color) {
    case 'RED': return 2;
    case 'GREEN': return 1;
...
const getBoardRotationAngle = (color) => {
  switch (color) {
    case 'RED': return '180deg';
    case 'GREEN': return '90deg';
    case 'YELLOW': return '-90deg';
    case 'BLUE':
    default:
      return '0deg';
  }
...
const getInverseRotationAngle = (color) => {
  switch (color) {
    case 'RED': return '180deg';
    case 'GREEN': return '-90deg';
    case 'YELLOW': return '90deg';
...
const getPerspectiveLayout = (myCol) => {
  let bottomColor = 'BLUE';
  let topColor = 'GREEN';
  let leftColor = 'RED';
  let rightColor = 'YELLOW';

  if (myCol === 'RED') {
    bottomColor = 'RED'; topColor = 'YELLOW'; leftColor = 'GREEN'; rightColor = 'BLUE';
  } else if (myCol === 'GREEN') {
    bottomColor = 'GREEN'; topColor = 'BLUE'; leftColor = 'RED'; rightColor = 'YELLOW';
  } else if (myCol === 'YELLOW') {
    bottomColor = 'YELLOW'; topColor = 'RED'; leftColor = 'BLUE'; rightColor = 'GREEN';
  }

  return { bottomColor, topColor, leftColor, rightColor };
};
...
export default function App() {
    ...
    const backAction = () => {
      ...
      if (gameMode) {
        Alert.alert('Exit Game', 'Return to Main Lobby?', [
          ...
          { text: 'Quit Match', style: 'destructive', onPress: handleExitGame },
        ]);
        return true;
      }
      return false;
  ...
  const handleTurnTimeout = () => {
    ...
    executeAutoTimeoutDiceRoll();
  };

  const executeAutoTimeoutDiceRoll = async () => {
    setIsRolling(true);
    playSound('dice');
  ...
  const getStrategicMoveIndex = (color, diceVal, validMoves) => {
    ...
    const homeMove = validMoves.find((idx) => playerPawns[idx] + diceVal === 56 || (playerPawns[idx] >= 50 && playerPawns[idx] + diceVal <= 56));
      ...
      if (pairs && Array.isArray(pairs)) {
        ...
        if (usersData && Array.isArray(usersData)) {
          ...
          const list = usersData.map(u => {
            ...
            const isOnline = (now - lastSeenTime) < 300000;
            return { 
              id: u.player_id, 
              name: u.name, 
              online: isOnline, 
  ...
  const sendFriendInvite = async (friend) => {
    try {
      const checkRes = await fetch(`${SUPABASE_REST_URL}/ludo_game_invites?from_id=eq.${currentUser.playerId}&to_id=eq.${friend.id}&status=eq.PENDING`, {
        headers: { 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` }
      });
      const existing = await checkRes.json();
      if (existing && existing.length > 0) {
        Alert.alert('Already Invited', `An active invite is already pending for ${friend.name}. Please wait!`);
        return;
      }
    } catch (e) {}
    ...
    try {
      ...
      setFriendsList(prev => prev.map(f => f.id === friend.id ? { ...f, isInvited: true } : f));
      setFriendsModal(false);
      setOnlineLobbyModal(true);
      Alert.alert('Invite Sent! 🎮', `Match invitation sent to ${friend.name}. You are in the Lobby!`);
    } catch (e) {
  ...
  const startCustomPassPlay = () => {
    let colors = ['BLUE', 'GREEN'];
    let defaultRoomPlayers = {};

    if (playType === 'SOLO') {
      if (selectedPlayerCount === 2) {
        colors = ['BLUE', 'GREEN'];
        defaultRoomPlayers = {
          BLUE: { name: currentUser?.name || 'Player 1' },
          GREEN: { name: 'Player 2' }
        };
        ...
        defaultRoomPlayers = {
          ...
          RED: { name: 'Player 2' },
          GREEN: { name: 'Player 3' },
          YELLOW: { name: 'Player 4' }
        };
      }
    } else {
      colors = ['BLUE', 'RED', 'GREEN', 'YELLOW'];
      defaultRoomPlayers = {
        BLUE: { name: currentUser?.name || 'Team A (1)' },
        GREEN: { name: 'Team A (2)' },
        RED: { name: 'Team B (1)' },
        YELLOW: { name: 'Team B (2)' }
      };
    }
  ...
  const startMatchFromLobby = async () => {
    ...
    if (ws.current && ws.current.readyState === WebSocket.OPEN) {
      ws.current.send(JSON.stringify({
        ...
        payload: {
          type: 'START_MATCH',
          data: { activeColors, playType, syncedRoomPlayers: roomPlayers, entryFee: selectedEntryFee, prizePool: totalPool, playerSlots }
        },
        ref: 'start_1'
      }));
    }
        ...
        if (message.event === 'broadcast') {
          ...
          if (type === 'CHECK_ROOM_EXISTS') {
            ...
            if (activeColors.length <= 2) {
              ...
              setShowPodiumBoard(true);
              setFinishedRankings([myColor, leftColor]);
              if (myColor === activeColors.find(c => c !== leftColor)) {
                addWinnerCoins(matchPrizePool);
              }
              updateUserGameStats(true);
            } else {
              const remainingActive = activeColors.filter(c => c !== leftColor);
            ...
            if (data.playerSlots) setPlayerSlots(data.playerSlots);
            setOnlineLobbyModal(false);
  ...
  const getTransformedCellCoordinates = (row, col, myCol) => {
    const offset = getPerspectiveOffset(myCol);
    let r = row;
    let c = col;
  ...
  // Unified perspective mapping for arrows matching player screen rotation
  const getTransformedArrowDetails = (row, col, myCol) => {
    // Logical arrow icons & colors mapped to board positions
    let baseArrowIcon = '';
    let baseArrowColor = '#000000';
    if (row === 7 && col === 0) { baseArrowIcon = '➔'; baseArrowColor = '#ef4444'; }
    else if (row === 0 && col === 7) { baseArrowIcon = '⬇'; baseArrowColor = '#16a34a'; }
    else if (row === 7 && col === 14) { baseArrowIcon = '⬅'; baseArrowColor = '#eab308'; }
    else if (row === 14 && col === 7) { baseArrowIcon = '⬆'; baseArrowColor = '#2563eb'; }

    const offset = getPerspectiveOffset(myCol);
    const arrowsCycle = ['➔', '⬇', '⬅', '⬆'];
    let currentIdx = arrowsCycle.indexOf(baseArrowIcon);
    if (currentIdx !== -1) {
      currentIdx = (currentIdx + offset) % 4;
      baseArrowIcon = arrowsCycle[currentIdx];
    }
    return { arrowIcon: baseArrowIcon, arrowColor: baseArrowColor };
  };
  ...
  const renderCell = (row, col) => {
    ...
    if (isArrow) {
      const details = getTransformedArrowDetails(row, col, myColor);
      arrowIcon = details.arrowIcon;
      arrowColor = details.arrowColor;
    }

    const screenCoord = getTransformedCellCoordinates(row, col, myColor);
    const inverseRot = getInverseRotationAngle(myColor);

    return (
      <View key={`${row}-${col}`} style={[styles.cell, { left: screenCoord.c * CELL_SIZE, top: screenCoord.r * CELL_SIZE, backgroundColor: bgColor }]}>
        {isStar && <Text style={[styles.starCleanText, { transform: [{ rotate: inverseRot }] }]}>☆</Text>}
        {isArrow && <Text style={[styles.arrowCleanText, { color: arrowColor, transform: [{ rotate: inverseRot }] }]}>{arrowIcon}</Text>}
      </View>
    );
  };
  ...
  const renderAllTokens = () => {
    ...
    ALL_COLORS.forEach((color) => {
      pawns[color].forEach((stepCount, idx) => {
        ...
        const screenCoord = getTransformedCellCoordinates(coords[0], coords[1], myColor);
        let finalLeft = screenCoord.c * CELL_SIZE + offsetX;
        let finalTop = screenCoord.r * CELL_SIZE - 7 + offsetY;
              ...
              {friendsList.length === 0 ? (
                ...
                friendsList.map((friend) => (
                  <View key={friend.id} style={styles.friendItemCardWrap}>
                    ...
                    <TouchableOpacity 
                      style={[styles.inviteFriendActionBtn, friend.isInvited && { backgroundColor: '#475569' }]} 
                      disabled={friend.isInvited}
                      onPress={() => sendFriendInvite(friend)}
                    >
                      <Text style={styles.inviteFriendActionText}>
                        {friend.isInvited ? '✓ Invited' : '🎮 Invite'}
                      </Text>
                    </TouchableOpacity>
                  </View>
                ))
        ...
        return (
          <View style={[styles.playerSquareActive, badgeStyle]}>
            ...
            <Text style={styles.slotPlayerNameText} numberOfLines={1}>
              {isMeControlling ? currentUser.name : `Local (${col})`}
            </Text>
      ...
      <View style={styles.dashboardContainer}>
        ...
        <SafeAreaView style={styles.fulfilledTopActionCenterBar}>
          <View style={styles.topActionCenterInnerRow}>
            <TouchableOpacity activeOpacity={0.85} style={styles.megaFulfilledButton} onPress={() => setLeaderboardModal(true)}>
              ...
              <Text style={styles.megaFulfilledText}>Leaderboard</Text>
            </TouchableOpacity>

            <TouchableOpacity activeOpacity={0.85} style={styles.megaFulfilledButton} onPress={() => setFriendsModal(true)}>
              <Text style={styles.megaFulfilledEmoji}>👥</Text>
              <Text style={styles.megaFulfilledText}>Friends Squad</Text>
            ...
            <TouchableOpacity activeOpacity={0.85} style={styles.megaFulfilledButton} onPress={() => setSettingsModal(true)}>
              <Text style={styles.megaFulfilledEmoji}>⚙️</Text>
              <Text style={styles.megaFulfilledText}>Settings</Text>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
        ...
        <View style={styles.centerBannerProfileWrapPerfect}>
          ...
          <TouchableOpacity activeOpacity={0.85} onPress={() => setProfileStatsModal(true)} style={styles.centerBannerContainerPerfect}>
            <Text style={styles.centerBannerUsername} numberOfLines={1}>
              {currentUser.name}
            </Text>
            <View style={styles.centerBannerCoinsRow}>
              <Text style={{ fontSize: 13, marginRight: 4 }}>🪙</Text>
              <Text style={styles.centerBannerCoinsText}>
