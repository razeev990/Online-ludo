...
export default function App() {
  ...
  // Fixed perspective logic based on user's assigned color (myColor)
  // If myColor is GREEN, we map the bottom card slot to GREEN instead of BLUE
  const bottomCardColor = myColor === 'GREEN' ? 'GREEN' : myColor === 'YELLOW' ? 'YELLOW' : myColor === 'RED' ? 'RED' : 'BLUE';
  const topCardColors = activeColors.filter(c => c !== bottomCardColor);
  ...
  const renderPlayerCard = (color, pinHex, shadowHex, isLeftDice = false) => {
    const isPlayable = activeColors.includes(color) && !finishedRankings.includes(color);
  ...
  return (
    <SafeAreaView style={styles.mainContainer}>
      ...
      {/* Dynamic Top Cards Layout (Shows opponents at the top based on active colors) */}
      <View style={styles.topCardsRow}>
        {topCardColors.map((col) => {
          const hex = getTurnColorHex(col);
          return (
            <React.Fragment key={col}>
              {renderPlayerCard(col, hex, hex, false)}
            </React.Fragment>
          );
        })}
      </View>
      ...
      {/* Dynamic Bottom Card Layout (Always shows the current logged-in player's card/dice at the bottom) */}
      <View style={styles.bottomCardsRow}>
        {renderPlayerCard(bottomCardColor, getTurnColorHex(bottomCardColor), getTurnColorHex(bottomCardColor), true)}
      </View>
    </SafeAreaView>
  );
}
