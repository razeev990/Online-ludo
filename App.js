...
export default function App() {
  ...
  // Fix: Online/Hybrid mein Guest ke liye color aur board flip mapping theek rakhein
  ...
  // Fix: 30s timeout hone par automatic AI best move aur 3 miss par disqualification
  const handleTurnTimeout = () => {
    ...
    // Automatically play best move on timeout
    const currentVal = playerDices[currentTurn] || Math.floor(Math.random() * 6) + 1;
    const validMoves = getValidMoves(currentTurn, currentVal);

    if (validMoves.length > 0) {
      const bestMove = getStrategicMoveIndex(currentTurn, currentVal, validMoves);
      executeStepMovement(currentTurn, bestMove, currentVal, playerDices);
    } else {
  ...
  // Fix: 2-player exit instant win, 3/4 player exit disconnect continue logic
        ...
        >
          {isLeftDice ? (
            <>
              ...
              {/* Fix: Gaping area mein 30s timer */}
              {isCurrent && (
  ...
  if (!gameMode) {
    ...
    return (
      <View style={styles.dashboardContainer}>
        ...
        {/* 2. PERFECTLY ALIGNED CENTER BANNER */}
