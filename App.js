...
// Board is rotated for each player's perspective. This chooses the local edge
// that always becomes the bottom edge on screen, so player names never move to a side.
const getPlayerLabelPositionStyle = (color) => {
  // Keep every player's name on the horizontal edge of its own base.
  // This prevents RED/YELLOW labels from moving to a side on rotated boards.
  const map = {
    BLUE: 'playerLabelBottom',
    RED: 'playerLabelTop',
    GREEN: 'playerLabelTop',
    YELLOW: 'playerLabelBottom'
  };

  return map[color] || 'playerLabelBottom';
};
...
const [dailyBonusClaimed, setDailyBonusClaimed] = useState(false);
  ...
  // Keep the latest dice values in a ref so async animation / WebSocket callbacks
  // never build a sync payload from a stale React state snapshot.
...
const handleTimeoutMiss = () => {
  ...
  if (
    ...
    updatePlayerDice(currentTurn, finalVal);
    await sleep(200);

    setIsRolling(false);
    setHasRolled(true);

    const newDices = { ...playerDicesRef.current, [currentTurn]: finalVal };
...
useEffect(() => {
  ...
  const renderBase = (color, posStyle, isVertical) => {
    ...
    const inverseRot = getInverseRotationAngle(myColor);
    const labelPositionStyle = styles[getPlayerLabelPositionStyle(color)];
