...
export default function App() {
  ...
  // LUDO KING STYLE PERSPECTIVE ROTATION: Agar guest GREEN hai, toh board coordinates ko 180 degree coordinate transform denge taaki board rotate na ho aur goti bilkul sahi jagah chale!
  const isGuestGreen = (gameMode === 'ONLINE' || gameMode === 'HYBRID') && myColor === 'GREEN';

  const transformCoords = (r, c) => {
    if (isGuestGreen) {
      return [14 - r, 14 - c];
    }
    return [r, c];
  ...
  const getPawnScreenCoords = (color, stepCount, idx) => {
    let baseCoords;
    if (stepCount === -1) baseCoords = BASE_SPOTS[color][idx];
    else if (stepCount === 56) baseCoords = [7, 7];
    else if (stepCount >= 51) baseCoords = HOME_PATHS[color][stepCount - 51];
    else baseCoords = TRACK_COORDINATES[(START_INDEX[color] + stepCount) % 52];

    return transformCoords(baseCoords[0], baseCoords[1]);
  };
  ...
  const renderCell = (row, col) => {
    ...
    const [tr, tc] = transformCoords(row, col);

    return (
      <View key={`${row}-${col}`} style={[styles.cell, { left: tc * CELL_SIZE, top: tr * CELL_SIZE, backgroundColor: bgColor }]}>
        {isStar && <Text style={styles.starCleanText}>☆</Text>}
        {isArrow && <Text style={[styles.arrowCleanText, { color: arrowColor }]}>{arrowIcon}</Text>}
      </View>
    );
  };
  ...
  const renderAllTokens = () => {
    ...
    ALL_COLORS.forEach((color) => {
      pawns[color].forEach((stepCount, idx) => {
        ...
        let finalLeft = coords[1] * CELL_SIZE + offsetX;
        let finalTop = coords[0] * CELL_SIZE - 7 + offsetY;
  ...
  // LUDO KING PERSPECTIVE MAPPING FOR PLAYER CARDS
  const topCard1 = isGuestGreen ? renderPlayerCard('YELLOW', '#eab308', '#a16207', false, true) : renderPlayerCard('RED', '#ef4444', '#b91c1c', false, true);
  const topCard2 = isGuestGreen ? renderPlayerCard('BLUE', '#2563eb', '#1d4ed8', true, true) : renderPlayerCard('GREEN', '#16a34a', '#15803d', true, true);
  
  const bottomCard1 = isGuestGreen ? renderPlayerCard('GREEN', '#16a34a', '#15803d', false, false) : renderPlayerCard('BLUE', '#2563eb', '#1d4ed8', false, false);
  const bottomCard2 = isGuestGreen ? renderPlayerCard('RED', '#ef4444', '#b91c1c', true, false) : renderPlayerCard('YELLOW', '#eab308', '#a16207', true, false);
