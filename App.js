...
export default function App() {
  ...
  // ========== BACK HANDLER FOR EXIT DIALOG ==========
  useEffect(() => {
    const backHandler = BackHandler.addEventListener('hardwareBackPress', () => {
      if (gameMode && !showPodiumBoard) {
        handleExitGame();
        return true;
      }
      return false;
    ...
    try {
      await fetch(`${SUPABASE_REST_URL}/ludo_users`, {
        ...
        body: JSON.stringify({
          ...
          email: userObj.email,
          coins: Number(userObj.coins || 2000), // changed to 2000
  ...
  const claimDailyBonus = async () => {
    ...
    try {
      ...
      const reward = 200; // changed to 200
  ...
  const renderAllTokens = () => {
    ...
    ALL_COLORS.forEach((color) => {
      pawns[color].forEach((stepCount, idx) => {
        ...
        // Center the token in its cell
        const centerOffset = CELL_SIZE / 2;
        const finalLeft = coords[1] * CELL_SIZE + offsetX - centerOffset;
        const finalTop = coords[0] * CELL_SIZE + offsetY - centerOffset;
                  ...
                  {cloudLeaderboardData.map((player, index) => {
                    ...
                    return (
                      <View key={player.player_id} style={[styles.leaderboardRowItem, isMe && styles.leaderboardHostRow]}>
                        <Text style={[styles.rankBadgeText, { flex: 0.3, fontSize: 12 }]}>{rankEmoji}</Text>
                        <Text style={[styles.rankPlayerName, { flex: 1, fontSize: 13 }, isMe && { color: '#facc15' }]}>{player.name} {isMe ? '(You)' : ''}</Text>
                        <Text style={[styles.rankCoinText, { flex: 0.4, textAlign: 'right' }]}>🪙 {player.coins.toLocaleString()}</Text>
                      </View>
                    );
                  })}
                </ScrollView>
