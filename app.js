...
export default function App() {
  ...
  const getStrategicMoveIndex = (color, diceVal, validMoves) => { /* ... unchanged ... */ };
  const getValidMoves = (color, diceVal) => { /* ... unchanged ... */ };
  ...
  const rollDice = async (isBot = false, isAutoTimeout = false) => { /* ... unchanged ... */ };
  const executeStepMovement = async (color, index, diceVal, currentDices = playerDices) => { /* ... unchanged ... */ };
  const finalizeMove = (color, index, finalStep, diceVal, finalState, currentDices) => { /* ... unchanged ... */ };
  ...
  useEffect(() => {
    ...
    socket.onopen = () => {
      ...
      // ✅ Only HOST broadcasts PLAYER_JOINED on open – guest sends after receiving assignedColor
    ...
    tempWs.onmessage = (e) => {
      try {
        ...
        if (message.payload?.type === 'ROOM_EXISTS_CONFIRMED') {
          ...
          // ✅ Now broadcast PLAYER_JOINED with correct color
  ...
  // Friends Squad Modal (inline function)
              ...
              {pendingRequests.length === 0 ? <Text style={styles.squadEmptyStateText}>No pending friend requests.</Text> :
                pendingRequests.map((req) => (
                  <View key={req.id} style={styles.friendItemCardWrap}>
                    <View><Text style={styles.friendCardNameText}>{req.name}</Text><Text style={styles.friendCardSubText}>Sent you a friend request</Text></View>
